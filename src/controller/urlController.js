...
let createUrl = async (req, res) => {
    try {
        ...
        if (urlCheck) {
            return res.status(200).send({ status: true, data: urlCheck ,msg:"shortUrl for this already exist"})
        }
...
const redirectUrl = async function (req, res) {
    try {
        ...
        if (!shortId.isValid(url)) {
            return res.status(400).send({ status: false, message: "Please Enter a valid short Url" })
        }
